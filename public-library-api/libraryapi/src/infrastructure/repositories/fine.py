...
class FineRepository(IFineRepository):
    ...
    async def list_unpaid_fines_by_user(self, user_id: UUID4) -> list[Fine]:
        query = fine_table.select().where(
            (fine_table.c.user_id == user_id) &
            (fine_table.c.status == "unpaid")
        )
        rows = await database.fetch_all(query)
        return [Fine(**row) for row in rows]

    async def mark_fine_as_paid(self, fine_id: UUID4) -> bool:
        query = fine_table.update().where(
            fine_table.c.id == fine_id
        ).values(status="paid")
        result = await database.execute(query)
        return result > 0
